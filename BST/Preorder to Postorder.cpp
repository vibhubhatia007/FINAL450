...
void solve(int pre[],int size,int min,int max,int &index)
{
    if(index==size)
    return ;
    ...
    solve(pre,size,min,val,index);
    solve(pre,size,val,max,index);
...
Node* constructTree(int pre[], int size) {
    Node *ans=NULL;
    int max=INT_MAX;int min= INT_MIN;int index=0;
    solve(pre,size,min,max,index);
